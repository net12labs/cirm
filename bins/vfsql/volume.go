...
// ListFilesystems returns all filesystem names in this volume
func (v *Volume) ListFilesystems() ([]string, error) {
	query := `SELECT name FROM filesystems WHERE volume_id = ? ORDER BY name`
	rows, err := v.db.db.Query(query, v.id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
